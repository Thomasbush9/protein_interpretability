...
def main():
    ...
    proto = pi_protocol.protocol(
        script="analyze_ablate.py",
        design="projection deletion: one direction removed from a REAL "
               "variant's dz = z_mut - z_wt at every pair, structure module "
               "re-run; PC2 vs PC1 and random directions, never vs nothing",
        layer=pi_protocol.layers("final"),
        features=pi_protocol.features("deleted direction, raw z units", 128,
                                      kept=1),
        source=a.glob, n_assays=len(names),
        statistic="distogram-width recovery fraction, d pLDDT, CA shift; "
                  "cluster bootstrap over assays, paired PC2 minus random",
        positive_control="residual projection after removal, reported per "
                         "direction — a null without it is uninterpretable")
    pi_archive.write_result(a.out, out, protocol=proto, indent=2)
    print(f"\nwrote {a.out}")
